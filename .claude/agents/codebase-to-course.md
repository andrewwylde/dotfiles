...
The `references/` directory is located at `~/.claude/agents/codebase-to-course-references/`. When the skill says `references/foo.md`, read from `~/.claude/agents/codebase-to-course-references/foo.md`. **Read them only when you reach the relevant phase** — not upfront. This keeps context lean.
