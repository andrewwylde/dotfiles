...
### 6. Save to PR Reviews viewer

After writing the review file, run:

```bash
# Get the PR title for the viewer
PR_TITLE=$(gh pr view <PR_NUMBER> --json title --jq '.title')
REPO=$(gh repo view --json nameWithOwner --jq '.nameWithOwner' 2>/dev/null || echo "")

~/code/pr-reviews/save-review.sh \
  <PR_NUMBER> \
  .context/prs/reviews/PR_REVIEW_#<PR_NUMBER>.md \
  .context/prs/diffs/pr<PR_NUMBER>.diff \
  "$PR_TITLE" \
  "$REPO"
```

This adds the review and diff to the local Mintlify viewer at `~/code/pr-reviews/`.
To browse reviews: `cd ~/code/pr-reviews && mintlify dev`
