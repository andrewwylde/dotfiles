...
for pattern in "${patterns[@]}"; do
  if [[ "$staged_only" -eq 1 ]]; then
    matches="$(git grep -n -E "$pattern" --cached -- . ':!bin/leak-check.sh' 2>/dev/null || true)"
  else
    matches="$(git grep -n -E "$pattern" -- . ':!bin/leak-check.sh' 2>/dev/null || true)"
  fi
  if [[ -n "$matches" ]]; then
