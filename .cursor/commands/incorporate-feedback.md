...
# Get all review comments (pending and resolved)
gh pr view <PR_NUMBER> --json reviewThreads --jq '.reviewThreads[] | select(.isResolved == false) | {path: .path, line: .line, comments: [.comments[] | {author: .author.login, body: .body, createdAt: .createdAt}]}'

# Get all review comments (including resolved for context)
gh pr view <PR_NUMBER> --json reviewThreads --jq '.reviewThreads[] | {path: .path, line: .line, isResolved: .isResolved, comments: [.comments[] | {author: .author.login, body: .body, createdAt: .createdAt}]}'
...
gh api repos/:owner/:repo/pulls/<PR_NUMBER>/comments --jq '.[] | {author: .user.login, body: .body, createdAt: .created_at}'
```

**Store this data as:**
- **PR Metadata**: number, title, branch, state
- **Review Threads**: All line-specific comments (resolved and unresolved)
- **Review Summaries**: Overall review states and comments
- **General Comments**: Comments not tied to specific code lines
...
### 11. Resolve review threads (optional)

After addressing feedback, mark threads as resolved:

```bash
# Get thread IDs
gh pr view <PR_NUMBER> --json reviewThreads --jq '.reviewThreads[] | select(.isResolved == false) | {id: .id, path: .path, line: .line}'

# Resolve thread (requires GitHub API)
gh api repos/:owner/:repo/pulls/<PR_NUMBER>/threads/<THREAD_ID>/resolve -X PUT
```

**Note:** Only resolve threads if the feedback has been fully addressed. If you need clarification, add a comment instead.
