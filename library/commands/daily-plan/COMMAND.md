...
### Output formatting (Slack + Notion + chat)

Apply these to the Draft Slack status, Notion journal body, and Step 8 chat summary:

- **No markdown tables.** Never use pipe-tables (`| col |`). Prefer bullets or one-line `Label — detail` items.
- **Links use standard markdown only:** `[short label](https://example.com/path)`
- **Banned link forms:** Slack mrkdwn `<https://...|label>`, bare `#NNNN` without a URL, unlabeled bare URLs, HTML `<a>` tags.
- Every PR mentioned (review queue and own shepherding PRs) needs a markdown link.
...
**PR links are required.** Every PR mentioned must use markdown links:

```
[#NNNN short title](https://github.com/parable-work/parable-platform/pull/NNNN)
```
...
* PR reviews: [#1542 connector auth](https://github.com/parable-work/parable-platform/pull/1542), [#1538 migration fix](https://github.com/parable-work/parable-platform/pull/1538)
* Shipping: [#4795 Docker types](https://github.com/parable-work/parable-platform/pull/4795)
...
*(not sent — copy the fenced block into Slack; use markdown links)*

~~~
[paste the same Draft Slack Status Message from Step 5 here, with [#NNNN title](url) links for every PR]
~~~
```
...
**Always write the Draft Slack status section into the Notion page** (same copy-paste body as Step 5 / Step 8), with markdown GitHub links for every PR mentioned. No pipe-tables anywhere in the journal body.
...
2. **Draft Slack status message** — formatted and ready to copy-paste, with markdown GitHub links (`[#NNNN title](url)`) for every PR. Remind the user this was NOT sent. Confirm the same draft was written into the Notion journal under **Draft Slack status**.
3. **Brief summary**: PR count in review queue (from filter script only), active ticket count, notable Slack activity highlights. Do not report total open PRs or stale search hits. Do not use markdown tables or Slack `<url|label>` links in the summary.
