...
### 8) Add compliance context if PII or regulated data is in scope

- If the system handles PII, payment data, health records, or other regulated data, add a brief compliance section noting applicable frameworks (e.g., GDPR/CCPA for personal data, PCI DSS for payment processing, HIPAA for health data, SOC 2 for B2B SaaS).
- For each framework: note whether the system is likely in scope, and call out 2–3 specific controls from the threat model that are also compliance evidence requirements.
- Keep it brief — the goal is to connect the security work to the compliance work, not to write a compliance assessment.

### 9) Run a quality check before finalizing
