...
// ---------------------------------------------------------------------------
// Auto-auth: detect URLs that need a project-specific login helper and load it.
// Register handlers in PROJECT_AUTH_HANDLERS (see autoAuth below).

const PROJECT_AUTH_HANDLERS = [];

/**
 * Detect whether a URL belongs to a known project that has a dev-login helper.
 * Returns the matching handler descriptor + URL match info, or null.
 *
 * @param {string} url - Target URL ppwt is about to navigate to
 * @returns {Object|null} { handler, match: { kind, tenant } } or null
 */
function detectAuthRequirement(url) {
  for (const handler of PROJECT_AUTH_HANDLERS) {
    const match = handler.detectUrl(url);
...
/**
 * Lazy-load a project's dev-login helper. Returns null with a console warning
 * if the helper script isn't installed.
 */
function loadAuthHelper(handler) {
  if (!fs.existsSync(handler.helperPath)) {
    console.warn(
      `[autoAuth] ${handler.name} helper not found at ${handler.helperPath}`
    );
    return null;
  }
  try {
    return require(handler.helperPath);
  } catch (e) {
    console.warn(`[autoAuth] failed to load ${handler.helperPath}:`, e.message);
    return null;
  }
}

/**
 * Auto-detect whether the target URL needs a project-specific dev login and
 * run it. Idempotent — safe to call at the top of every script. Returns:
 *   { handled: boolean, project?: string, user?: string }
 *
 * Defaults:
 *   - admin origins → "Sarah Administrator"
 *   - tenant origins → "Alice Johnson"
 *   - dismissPanel: true (Escape + click neutral corner)
 *
 * Override via opts:
 *   { user: 'Diana Prince', dismissPanel: false }
 *
 * @param {Object} page - Playwright page
 * @param {string} url - Target URL
 * @param {Object} [opts]
 * @returns {Promise<Object>} {handled, project?, user?}
 */
async function autoAuth(page, url, opts = {}) {
  const detected = detectAuthRequirement(url);
  if (!detected) return { handled: false };

  const { handler, match } = detected;
  const helper = loadAuthHelper(handler);
  if (!helper || typeof helper.devLogin !== 'function') {
    return { handled: false };
  }

  const user = opts.user || handler.defaultUser(match);
  console.log(`[autoAuth] ${handler.name}/${match.kind}: logging in as "${user}"`);
  await helper.devLogin(page, url, user);
  if (opts.dismissPanel !== false && typeof helper.dismissDevPanel === 'function') {
    await helper.dismissDevPanel(page);
  }
  return { handled: true, project: handler.name, user };
}
