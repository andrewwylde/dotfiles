...
This skill produces the **split plan** with a typed hand-off block. To execute the plan (create branches, stage hunks, push, open PRs), hand off to **split-to-prs**. That skill routes independent PRs itself and loads **gh-stack** for HARD linear stack groups.
...
5. **Deliver:** Produce a concrete split plan — named PRs, file lists per PR, merge order, stacking dependencies, and **execution groups** (`executor: split-to-prs` vs `executor: gh-stack`) — tied to actual paths in the diff, not generic advice.
...
and a typed Hand-off to split-to-prs block (execution groups + file lists).
```
...
use Pattern 5 to produce a stacked PR chain diagram, linearize for gh-stack,
and emit execution groups in the hand-off.
```
...
**Pattern 6** (branch creation, staging, committing, pushing, opening PRs) is not duplicated here. After Pattern 5 produces the split plan, switch to **split-to-prs** to execute it.

`split-to-prs` is the sole executor:

- **`executor: split-to-prs`** groups — independent PRs off the default branch (existing snapshot / branch / `gh pr create` flow).
- **`executor: gh-stack`** groups — load the **gh-stack** skill and run non-interactive `gh stack` commands for HARD linear chains.
- Soft dependencies stay as merge-order notes; they never become stack membership.
- If a stack group is marked `linear: false`, `split-to-prs` warns and asks before opening (do not invent a false linear order here).
