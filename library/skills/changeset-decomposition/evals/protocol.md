...
Treatment output should end with a **split-to-prs hand-off** block that includes **Execution groups** (`executor: split-to-prs` and/or `executor: gh-stack`). A second eval can feed that block to an agent with `split-to-prs` and check it does not run git until user approval, and that stack groups load `gh-stack` (per that skill's rules).
